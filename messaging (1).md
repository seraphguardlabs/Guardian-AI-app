...
The gamified permission system allows guardians to assign tasks to children before granting time extensions. When a child completes the assigned task, the time extension is automatically approved.
...
3. Child completes the task → status = `approved`, requested hours automatically granted
4. Guardian is notified via WebSocket of auto-approval
...
- **Method:** `POST`
- **Auth:** Child (header)
- **Description:** Mark a task as completed. If the task is linked to a time extension request (via `task_assigned` status), the time extension is automatically approved.
...
**Response (task linked to time extension request - auto-approved):**
```json
{
    "status": "ok",
    "message": "Task completed - time extension auto-approved",
    "task": {
        "id": 15,
        "title": "Play Chess",
        "description": "Engage in a game of chess",
