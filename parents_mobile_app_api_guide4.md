...
Exam Mode allows guardians to completely block specific apps on a child's device during important study periods or exams. When enabled, all apps in the exam mode list will have their screen time limit set to 0 hours, effectively blocking them.

### 9. Exam Mode Settings

**URL:** `GET/POST /api/mobile/child/<child_hash>/exam-mode/`

**Headers:**
- `X-Email`: Guardian's email
- `X-Password`: Guardian's password

#### GET - Retrieve Current Exam Mode Settings
...
#### POST - Update Exam Mode Settings

##### Option 1: Toggle Exam Mode On/Off
...
```json
{
  ...
  "child_hash": "abc123xyz",
  "exam_mode": true,
  "exam_mode_apps": ["com.instagram.android", "com.snapchat.android"],
  "total_apps": 2,
  "updated_fields": ["exam_mode"]
}
```

##### Option 2: Set Apps to Block During Exam Mode
...
```json
{
  ...
  "total_apps": 4,
  "updated_fields": ["exam_mode_apps"]
}
```

##### Option 3: Enable Exam Mode and Set Apps Together
...
##### Option 4: Add a Single App to Exam Mode List
...
##### Option 5: Remove an App from Exam Mode List
...
### How Exam Mode Works with Restricted Apps

When exam mode is enabled (`exam_mode: true`), the restricted apps endpoint will return 0 hours for all apps in the `exam_mode_apps` list. This takes precedence over any regular time limits set in `restricted_apps`.

**Example:**
- Regular restriction: Instagram = 2 hours/day
- Exam mode apps: ["com.instagram.android"]
- With `exam_mode: false`: Instagram allowed 2 hours
- With `exam_mode: true`: Instagram allowed 0 hours (blocked)

**GET `/api/blocked-apps/<child_hash>/` Response (with exam mode ON):**
```json
{
  "child_hash": "abc123xyz",
  "restricted_apps": {
  ...
  "exam_mode": true,
  "exam_mode_apps": ["com.instagram.android"],
  "status": "success"
}
```

### Typical Exam Mode Workflow

1. **Setup (before exams):** Configure which apps to block
   ```json
   POST {"exam_mode_apps": ["com.instagram.android", "com.snapchat.android", "com.zhiliaoapp.musically"]}
   ```

2. **Enable (when exam starts):** Turn on exam mode
   ```json
   POST {"exam_mode": true}
   ```

3. **Disable (when exam ends):** Turn off exam mode
   ```json
   POST {"exam_mode": false}
   ```
...
#### Set Child's Public Key (Client-Side Key Generation)

**Endpoint:** `POST /api/mobile/child/<child_hash>/public-key/`

Use this when the child device generates its own key pair locally.
...
#### Set Guardian's Public Key (Client-Side Key Generation)

**Endpoint:** `POST /api/mobile/guardian/public-key/`

Use this when the guardian device generates its own key pair locally.
...
| Site Access | `GET /api/mobile/child/<hash>/site-access/` | Site logs |
| Restricted Apps | `GET/POST /api/mobile/child/<hash>/restricted-apps/` | Manage app restrictions |
| Exam Mode | `GET/POST /api/mobile/child/<hash>/exam-mode/` | Toggle exam mode & manage blocked apps |
| Child Public Key | `GET/POST /api/mobile/child/<hash>/public-key/` | Get or set child's encryption key |
| Guardian Public Key | `GET/POST /api/mobile/guardian/public-key/` | Get or set guardian's encryption key |
| Guardian Public Key (by ID) | `GET/POST /api/mobile/guardian/<id>/public-key/` | Get or set specific guardian's key |
