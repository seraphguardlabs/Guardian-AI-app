...
|-----------|------|---------|-------------|
| `status` | string | `pending` | Filter by status: `pending`, `task_assigned`, `approved`, `denied`, `responded`, `all` |
    ...
    "requests": [
        {
            ...
            "granted_hours": null,
            "created": "2026-02-22T10:30:00+00:00",
            "responded_at": "2026-02-22T10:35:00+00:00",
            "assigned_task": {
                "id": 15,
                "title": "Play Chess",
...
The gamified permission system allows guardians to assign tasks to children before granting time extensions. When a child completes the assigned task, the guardian is notified and can then approve the time extension.

### Flow
1. Child requests time extension → status = `pending`
2. Guardian assigns a reward task → status = `task_assigned`, task linked to request
3. Child completes the task → Guardian is notified via WebSocket
4. Guardian approves the time extension → status = `approved`, hours granted
...
- **Method:** `POST`
- **Auth:** Guardian (header)
- **Description:** Assign a task to a child for a pending time extension request. The request status changes to `task_assigned`. When the child completes the task, the time extension is automatically approved.

**Request (using suggested task):**
```json
{
    "task_id": "play_chess"
}
...
**Request (using custom task):**
```json
{
    "title": "Complete math homework",
    "description": "Finish all exercises on page 42"
}
```

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `task_id` | string | **Yes** (or title+description) | ID of suggested task: `play_chess`, `exercise`, `read` |
| `title` | string | **Yes** (or task_id) | Custom task title |
| `description` | string | **Yes** (or task_id) | Custom task description |

**Response (201 Created):**
```json
{
    "status": "ok",
    "message": "Reward task assigned successfully",
    "request_id": 42,
    "child_hash": "abc123def456",
    "child_name": "John Doe",
    "app_domain": "youtube.com",
...
| Status | Message |
|--------|--------|
| 400 | `Request already task_assigned` |
| 400 | `Provide task_id OR both title and description` |
| 400 | `Invalid task_id. Choose from: ['play_chess', 'exercise', 'read']` |
...
- **Method:** `POST`
- **Auth:** Guardian (header)
- **Description:** Directly approve a pending time extension request without assigning a task.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `granted_hours` | number | No | Hours to grant (defaults to `requested_hours`) |
| `response_encrypted` | string | No | Optional encrypted message for the child |

**Response:**
```json
{
    "status": "ok",
    "message": "Time extension approved",
    "request_id": 42,
    "child_hash": "abc123def456",
    "child_name": "John Doe",
    "app_domain": "youtube.com",
...
| Status | Message |
|--------|--------|
| 400 | `Request already approved` |
| 400 | `granted_hours must be between 0 and 24` |
| 404 | `Request not found or unauthorized` |
...
All chat messages are end-to-end encrypted. The server stores only encrypted content (`message_encrypted`) and never reads the plain text. Messages sent by a guardian are encrypted with the child's public key; messages sent by a child are encrypted with the guardian's public key. The recipient decrypts on-device using their private key.

### 4.1   Guardian → Child Chat

- **URL:** `/api/mobile/child/<child_hash>/chat/`
- **Methods:** `GET`, `POST`
- **Auth:** Guardian (header) — `X-Email` + `X-Password`
- **Rate Limit:** Mobile API throttle

#### GET — Retrieve chat messages

Returns the conversation between the authenticated guardian and the specified child, with pagination, optional auto-mark-read, and unread count.

**Path Parameters:**

| Parameter | Type | Description |
|-----------|------|-------------|
| `child_hash` | string | The child's unique hash identifier |

**Query Parameters:**

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `limit` | integer | 50 | Maximum number of messages to return (capped at 100) |
| `before_id` | integer | — | Return messages with `id < before_id` (backward pagination). Results are ordered oldest-first. |
| `after_id` | integer | — | Return messages with `id > after_id` (forward / real-time polling). Results are ordered oldest-first. |
| `mark_read` | string | `false` | Set to `true` to automatically mark all retrieved child-sent messages as read |

> **Pagination note:** When `before_id` is provided, the server fetches messages ordered by `-created` (newest first) then reverses the list so the final response is always in chronological (oldest-first) order. When `after_id` is provided, messages are ordered by `created` (oldest first). If neither is provided, the most recent `limit` messages are returned in chronological order.
    ...
    "messages": [
        {
            "id": 100,
            "sender_type": "child",
            "message_encrypted": "base64_encrypted_message_from_child",
            "is_read": true,
            "created": "2026-02-22T10:30:00+00:00"
        },
        {
            ...
            "message_encrypted": "base64_encrypted_message_from_guardian",
            "is_read": false,
            "created": "2026-02-22T10:35:00+00:00"
        }
    ],
...
| `messages[].id` | integer | Unique message ID (use for pagination with `before_id`/`after_id`) |
| `messages[].sender_type` | string | `"guardian"` or `"child"` |
| `messages[].message_encrypted` | string | Base64-encoded encrypted message content |
| `messages[].is_read` | boolean | Whether the recipient has read the message |
| `messages[].created` | string | ISO 8601 timestamp of when the message was sent |
| `unread_count` | integer | Number of unread messages from this child (after any mark-read operation) |
| `marked_read` | integer | Number of messages marked as read during this request (0 if `mark_read` was not `true`) |
| `count` | integer | Number of messages returned in this response |

**Error Responses:**

| Status | Condition | Response |
|--------|-----------|----------|
| 401 | Missing or invalid `X-Email`/`X-Password` headers | `{"status": "error", "message": "Authentication required. Provide X-Email and X-Password headers."}` |
| 401 | Wrong credentials | `{"status": "error", "message": "Invalid credentials"}` |
| 403 | Guardian is not linked to this child | `{"status": "error", "message": "You do not have access to this child"}` |
| 404 | Child hash does not exist | `{"status": "error", "message": "Child not found"}` |

---

#### POST — Send a message to child

Sends an encrypted message from the guardian to the child. The `message_encrypted` field should contain the message encrypted with the child's public key.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `message_encrypted` | string | **Yes** | The encrypted message content (base64-encoded, encrypted with child's public key) |
...
| Field | Type | Description |
|-------|------|-------------|
| `message_id` | integer | The unique ID of the created message |
| `child_hash` | string | The child the message was sent to |
| `guardian_id` | integer | The guardian who sent the message |
| `created` | string | ISO 8601 timestamp of when the message was created |

**Error Responses:**

| Status | Condition | Response |
|--------|-----------|----------|
| 400 | Missing `message_encrypted` field | `{"status": "error", "message": "message_encrypted is required"}` |
| 401 | Missing or invalid guardian credentials | `{"status": "error", "message": "Authentication required. Provide X-Email and X-Password headers."}` |
| 403 | Guardian is not linked to this child | `{"status": "error", "message": "You do not have access to this child"}` |
| 404 | Child hash does not exist | `{"status": "error", "message": "Child not found"}` |

---

### 4.2   Child → Guardian Chat

- **URL:** `/api/mobile/guardian/<guardian_id>/chat/`
- **Methods:** `GET`, `POST`
- **Auth:** Child (header) — `X-Child-Hash`
- **Rate Limit:** Mobile API throttle

#### GET — Retrieve chat messages

Returns the conversation between the authenticated child and the specified guardian, with pagination, optional auto-mark-read, and unread count.

**Path Parameters:**

| Parameter | Type | Description |
|-----------|------|-------------|
| `guardian_id` | integer | The guardian's database ID |
...
|-----------|------|---------|-------------|
| `limit` | integer | 50 | Maximum number of messages to return (capped at 100) |
| `before_id` | integer | — | Return messages with `id < before_id` (backward pagination) |
| `after_id` | integer | — | Return messages with `id > after_id` (forward / real-time polling) |
| `mark_read` | string | `false` | Set to `true` to automatically mark all retrieved guardian-sent messages as read |
...
Response fields are identical to [4.1 GET](#get--retrieve-chat-messages).

**Error Responses:**

| Status | Condition | Response |
|--------|-----------|----------|
| 401 | Missing `X-Child-Hash` header | `{"status": "error", "message": "Authentication required. Provide X-Child-Hash header."}` |
| 401 | Invalid child hash | `{"status": "error", "message": "Invalid child hash"}` |
| 403 | Child is not linked to this guardian | `{"status": "error", "message": "You do not have access to this guardian"}` |
| 404 | Guardian ID does not exist | `{"status": "error", "message": "Guardian not found"}` |

---

#### POST — Send a message to guardian

Sends an encrypted message from the child to the guardian. The `message_encrypted` field should contain the message encrypted with the guardian's public key.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `message_encrypted` | string | **Yes** | The encrypted message content (base64-encoded, encrypted with guardian's public key) |
...
### 4.3   Mark Messages Read

- **URL:** `/api/mobile/child/<child_hash>/chat/mark-read/`
- **Method:** `POST`
- **Auth:** Guardian (header) **or** Child (header)
- **Rate Limit:** Mobile API throttle
- **Description:** Mark messages as read. When a guardian calls this endpoint it marks child-sent messages as read; when a child calls it, it marks guardian-sent messages as read. Supports marking all unread messages or a specific set of message IDs.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `message_ids` | array of integers | No | Specific message IDs to mark as read. If omitted, **all** unread messages in the conversation are marked as read. |
| `guardian_id` | integer | **Yes** (child auth only) | Required when authenticating as a child. Identifies which guardian's conversation to mark as read. Not needed when authenticating as guardian. |
...
| Field | Type | Description |
|-------|------|-------------|
| `marked_read` | integer | Number of messages that were marked as read |
| `child_hash` | string | The child involved in the conversation |

**Error Responses:**

| Status | Condition | Response |
|--------|-----------|----------|
| 400 | Child auth without `guardian_id` in body | `{"status": "error", "message": "guardian_id is required for child authentication"}` |
| 401 | No valid auth headers provided | `{"status": "error", "message": "Authentication required"}` |
| 403 | Guardian not linked to child | `{"status": "error", "message": "You do not have access to this child"}` |
| 403 | Child hash mismatch (child auth) | `{"status": "error", "message": "Child hash mismatch"}` |
| 403 | Guardian not linked to child (child auth) | `{"status": "error", "message": "Guardian not linked to this child"}` |

**Authentication logic:**
1. The endpoint first attempts guardian authentication (via `X-Email` + `X-Password`).
2. If guardian auth fails, it attempts child authentication (via `X-Child-Hash`).
3. If both fail, a 401 is returned.
4. For guardian auth: marks messages sent by the child as read.
5. For child auth: marks messages sent by the guardian as read (requires `guardian_id` in request body).

---

### 4.4   Unread Message Counts

- **URL:** `/api/mobile/chat/unread/`
- **Method:** `GET`
- **Auth:** Guardian (header) **or** Child (header)
- **Rate Limit:** Mobile API throttle
- **Description:** Returns unread message counts across all conversations for the authenticated user. Useful for badge counts in mobile app UI.

**Authentication logic:** Same dual-auth as [4.3 Mark Messages Read](#43---mark-messages-read) — tries guardian first, falls back to child.

#### Response (Guardian — 200 OK):
...
| Field | Type | Description |
|-------|------|-------------|
| `user_type` | string | Always `"guardian"` for guardian auth |
| `conversations` | array | One entry per linked child |
| `conversations[].child_hash` | string | The child's unique hash |
| `conversations[].child_name` | string | The child's full name |
| `conversations[].unread_count` | integer | Number of unread messages from this child |
| `total_unread` | integer | Sum of all unread counts across all children |

#### Response (Child — 200 OK):
...
| Field | Type | Description |
|-------|------|-------------|
| `user_type` | string | Always `"child"` for child auth |
| `child_hash` | string | The authenticated child's hash |
| `conversations` | array | One entry per linked guardian |
| `conversations[].guardian_id` | integer | The guardian's database ID |
| `conversations[].guardian_name` | string | The guardian's full name |
| `conversations[].guardian_email` | string | The guardian's email address |
| `conversations[].unread_count` | integer | Number of unread messages from this guardian |
| `total_unread` | integer | Sum of all unread counts across all guardians |
...
- **Method:** `POST`
- **Auth:** Child (header)
- **Description:** Mark a task as completed. If the task is linked to a time extension request (via `task_assigned` status), the guardian is notified via WebSocket to approve the time extension.

**Response (regular task):**
```json
{
    "status": "ok",
...
**Response (task linked to time extension request - guardian notified):**
```json
{
    "status": "ok",
    "message": "Task completed. Guardian will be notified to approve the time extension.",
    "task": {
        "id": 15,
        "title": "Play Chess",
        "description": "Engage in a game of chess",
...
**WebSocket Notification to Guardian:**
When a linked task is completed, the guardian receives a WebSocket message:
```json
{
    "type": "task_completed",
    "data": {
        ...
        "app_domain": "youtube.com",
        "requested_hours": 2,
        "message": "Child completed the assigned task. Approve time extension?"
    }
}
```
...
| `child_name` | string | No | Child's name (defaults to child's full name if omitted) |
